  ...
  <div class="glass-card p-3.5 sm:p-5 border border-zinc-800 space-y-3.5 flex flex-col h-[680px] max-h-[88vh] relative">
    ...
    <div class="flex flex-col gap-2 pb-2.5 border-b border-zinc-800/80 shrink-0">
      ...
      <div class="flex items-center gap-1.5 overflow-x-auto py-1 pr-1 scrollbar-none text-xs">
        ...
        >
          <Radio class="w-3.5 h-3.5" :class="!selectedRecipientId ? 'text-zinc-950' : 'text-emerald-400'" />
          <span>Todos (Comunidad)</span>
          <span :class="['ml-0.5 px-1.5 py-0.2 rounded-full text-[10px] font-mono', !selectedRecipientId ? 'bg-zinc-950/30 text-zinc-950 font-bold' : 'bg-black/50 text-zinc-300 border border-zinc-800']">
        ...
        >
          ...
          ></span>
          <span class="truncate max-w-[110px]">{{ user.name }}</span>
          <span :class="['ml-0.5 px-1.5 py-0.2 rounded-full text-[10px] font-mono', selectedRecipientId === user.id ? 'bg-zinc-950/30 text-zinc-950 font-bold' : 'bg-black/50 text-zinc-300 border border-zinc-800']">
            {{ getPrivateCountForUser(user.id) }}
          </span>
        </button>
      </div>
    </div>
    ...
    >
      ...
      <template v-for="(group, gIdx) in groupedMessages" :key="group.dateLabel">
        ...
        >
          ...
          <div
            :class="[
              'px-3.5 py-2.5 rounded-2xl text-xs space-y-1.5 shadow-lg relative break-words',
              b.senderId === authStore.userId
                ? 'bg-emerald-800 text-white border border-emerald-500/60 rounded-br-xs'
                : 'bg-zinc-900 text-zinc-100 border border-zinc-700/80 rounded-bl-xs'
            ]"
          >
            ...
            <div v-if="b.senderId !== authStore.userId" class="flex items-center justify-between gap-2 pb-0.5">
              ...
              <span v-if="b.recipientId" class="text-[9px] px-1.5 py-0.2 rounded bg-emerald-950 text-emerald-300 border border-emerald-500/40 font-mono">Privado</span>
            </div>

            <!-- Quoted / Reply Preview (if message is a reply) -->
            <div 
              v-if="b.replyTo"
              class="p-2 rounded-xl bg-black/50 border-l-3 border-emerald-400 text-[11px] space-y-0.5 opacity-95 cursor-pointer"
            ...
            <div v-if="b.type === 'audio' && b.audioUrl" class="py-1 space-y-1.5 min-w-[200px] sm:min-w-[240px]">
              <div class="flex items-center gap-2.5">
                ...
                <Volume2 class="w-3.5 h-3.5 opacity-80 shrink-0" />
              </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-1.5 text-[10px] font-mono pt-0.5" :class="b.senderId === authStore.userId ? 'text-zinc-200' : 'text-zinc-400'">
              ...
              <template v-if="b.senderId === authStore.userId">
                <!-- 1. Pending (Relojito Ámbar) -->
                <span v-if="b.status === 'pending'" title="En cola offline. Se transmitirá al conectar.">
                  <Clock class="w-3.5 h-3.5 text-amber-300 animate-spin" />
                </span>
                <!-- 2. Sent (1 Check Gris) -->
                <span v-else-if="b.status === 'sent'" title="Transmitido a la red">
                  <Check class="w-3.5 h-3.5 text-zinc-300" />
                </span>
                <!-- 3. Delivered (2 Checks Grises Claros) -->
                <span v-else-if="b.status === 'delivered'" title="Entregado al dispositivo del contacto">
                  <CheckCheck class="w-3.5 h-3.5 text-zinc-200" />
                </span>
                <!-- 4. Read (2 Checks Azul Celeste / Cyan Luminoso Característico de WhatsApp) -->
                <span v-else-if="b.status === 'read'" title="Leído / Visto">
                  <CheckCheck class="w-3.5 h-3.5 text-cyan-300 font-bold drop-shadow-[0_0_5px_rgba(103,232,249,0.9)]" />
                </span>
              </template>
            </div>
          </div>
...
const publicBroadcastsCount = computed(() => {
  return (meshStore.broadcasts || []).filter(b => !b.recipientId).length;
});

const getPrivateCountForUser = (userId) => {
  const myId = authStore.userId;
  return (meshStore.broadcasts || []).filter(b => 
    (b.senderId === myId && b.recipientId === userId) ||
    (b.senderId === userId && b.recipientId === myId)
  ).length;
};

const isUserConnected = (userId) => {
  return meshStore.peerConnections.some(c => c && c.peer === userId && c.open);
};
